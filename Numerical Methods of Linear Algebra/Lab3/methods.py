    ...
    def __mul__(self, arg):
        ...
        if type(arg) == Vector:
            if len(arg) == self.width:
                ...
                raise ValueError(f"Несоответствующая размерность при попытке перемножения матрицы и вектора! {self.width} != {len(arg)}';\n self shape: ({self.height}, {self.width}), arg shape: ({len(arg)})")
        # умножение матрицы на матрицу
            ...
            if self.width == arg.height:
                ...
                raise ValueError(f"Несоответствие размерностей матриц при попытке перемножения! {self.width} != {arg.height}';\n self shape: ({self.height}, {self.width}), arg shape: ({arg.height}, {arg.width})")
...
def random_Matrix(n, a, b):
    ...
    for i in range(n):
        for j in range(n):
            res.matrix[i][j] = random.uniform(a, b)
    return res

def get_dd_matrix(n, a, b, dom = 1):
    res = random_Matrix(n, a, b)
    for i in range(n):
        res[i][i] = sum([abs(res[i][j]) for j in range(n)]) * 10 ** dom
    return res
...
def random_Vector(n, a, b):
    return Vector([random.uniform(a, b) for _ in range(n)])
