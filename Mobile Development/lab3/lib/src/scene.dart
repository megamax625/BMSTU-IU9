...
class Scene {
  ...
  void _calculateVertices(obj.Object o) {
    vertexCount += o.mesh.vertices.length;
    faceCount += o.mesh.indices.length;
    final List<obj.Object> children = o.children;
  ...
  void _renderObject(RenderMesh renderMesh, obj.Object o, Matrix4 model, Matrix4 view, Matrix4 projection) {
    ...
    final List<mesh.Polygon?> renderIndices = renderMesh.indices;
    final List<mesh.Polygon> indices = o.mesh.indices;
    final int indexOffset = renderMesh.indexCount;
    final int indexCount = indices.length;
    final bool culling = o.backfaceCulling;
    for (int i = 0; i < indexCount; i++) {
      final mesh.Polygon p = indices[i];
      ...
      if (!culling || !_isBackFace(ax, ay, bx, by, cx, cy)) {
        if (!_isClippedFace(ax, ay, az, bx, by, bz, cx, cy, cz)) {
          ...
          renderIndices[indexOffset + i] = mesh.Polygon(vertex0, vertex1, vertex2, sumOfZ);
        }
      }
    }
    ...
    if (o.lighting) {
      ...
      final mat.Material material = o.mesh.material;
    ...
    // render children
    List<obj.Object> children = o.children;
    for (int i = 0; i < children.length; i++) {
  ...
  void render(Canvas canvas, Size size) {
    ...
    final List<mesh.Polygon> renderIndices = <mesh.Polygon>[];
    final List<mesh.Polygon?> rawIndices = renderMesh.indices;
    for (int i = 0; i < rawIndices.length; i++) {
      final mesh.Polygon? p = rawIndices[i];
      if (p != null) renderIndices.add(p);
    }
    if (renderIndices.length == 0) return;

    // sort the faces by z
    renderIndices.sort((mesh.Polygon a, mesh.Polygon b) {
    ...
    // convert mesh.Polygon list to Uint16List
    ...
    for (int i = 0; i < indexCount; i++) {
      ...
      final mesh.Polygon polygon = renderIndices[i];
  ...
  void objectCreated(obj.Object object) {
    updateTexture();
  ...
  void _getAllMesh(List<mesh.Mesh> meshes, obj.Object object) {
    meshes.add(object.mesh);
    final List<obj.Object> children = object.children;
    for (int i = 0; i < children.length; i++) {
  ...
  void _updateTexture() async {
    ...
    texture = await packingTexture(meshes.cast<Mesh>());
    update();
  }
