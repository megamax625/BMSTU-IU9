...
def main():
    ...
    window = glfw.create_window(300, 300, "Lab8", None, None)  # создание окно
    if not window:
        glfw.terminate()
    ...
    fragment = """
                ...
                void main() {
                    gl_FragColor = texture(image, texCoordOut) * vertex_color;
                }"""
    program = load_program(vertex, fragment)
...
def display(window):  # функция, осуществляющая отрисовку основной призмы
    ...
    if fall:
        ...
        if minPointY <= -1 + velocity + 0.4:
            velocity = -1 * velocity
            fallpos += velocity
        glTranslatef(0, velocity, 0)  # сдвиг объекта
...
def displayPrism():
    ...
    for i in range(7):
        texCoords.append((math.cos(math.radians(52 * i)) + 1.0) / 2)
        texCoords.append((math.sin(math.radians(52 * i)) + 1.0) / 2)
    for n in range(len(basepoints) + len(cappoints)):
        if n < 6 or (6 < n < 13):
    ...
    colors = []
    for i in range(42):
        colors.append(0.3 + 0.015 * i)
        colors.append(0.9 - 0.01 * i)
        colors.append(0.1 + 0.015 * i)
